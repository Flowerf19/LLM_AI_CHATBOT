...
async def _run_stress_test(num_msgs=200):
    mock_bot = MagicMock()
    ...
    gs.GeminiService.return_value.generate_response = AsyncMock(return_value='{"basic_info": {"name": "Stress"}}')

    service = LLMMessageService(mock_bot)

    # Count handle invocations
    count = {'count': 0}
...
def test_stress():
    result = asyncio.run(_run_stress_test(200))
    assert result == 200, f"Expected 200 processed messages, got {result}"
